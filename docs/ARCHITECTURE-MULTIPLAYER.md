...
>
> **退役指针（2026-09-13）**：本文是 ADR-0002 落地前后的**迁移设计文档**。文中描述迁移前状态的段落（如 §一「现状是客户端权威架构」、gameStore/kpSessionService/客户端 toolCalling 的迁移表）均为**史实**，非现行形态——相关模块已按 ADR-0002 删除或收口。现行架构以 `CONTEXT.md`、`docs/adr/0002-solo-room.md`、`docs/ONBOARDING-GUIDE.md` §9 为准；本文 §四「服务端权威单轨」的原则仍然有效（CONTEXT.md「不重议的决策」引用）。
...
| FR-M12 | 旧存档迁移：saves 表历史快照导入单人房间；房间快照可导出 —— 已作废（2026-09-13，#92 / ADR-0008）：`/api/saves*` 全链退役，存量 saves 表恒空无害、不做迁移 | P1 | 数据在服务端，无客户端包袱 |
...
| NFR-M7 | 兼容性 | 单人玩法行为不回归（无旧客户端，仅保单人体验）；旧存档可导入（2026-09-13 追记：该子项随 `/api/saves*` 全链退役作废，见 #92 / ADR-0008） | 兼容对象从"客户端"变为"单人体验+数据" |
...
└──────────────────────────────────┬─────────────────────────────────────────────┘
                                   │ REST（auth/settings/stories/rag/saves/rooms；saves 已于 2026-09-13 退役，#92 / ADR-0008）
                                   │ WS（房间事件流）
┌──────────────────────────────────┴─────────────────────────────────────────────┐
│                             服务端（唯一权威）                                    │
...
- 旧 saves 表历史数据 → 导入单人房间（P1 迁移工具，双向：房间快照可导出为旧格式）。（2026-09-13 追记：该迁移候选已随 #91/#92 `/api/saves*` 全链退役作废，见 ADR-0008。）
...
| A5 | 存档迁移：saves 旧快照 ↔ 会话快照互转（2026-09-13 追记：已随 #92 / ADR-0008 `/api/saves*` 退役作废，saveService.ts 已删除） | saveService.ts |
...
| C3 | 旧存档导入/房间导出工具（2026-09-13 追记：已随 #92 / ADR-0008 退役作废） |
| C4 | 房主控制完善 + 房间级结局共享 |
