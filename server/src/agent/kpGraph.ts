...
function analyzeToolContinuation(messages: KpMessage[]): { isContinuation: boolean; followUpTools: string[] } {
  ...
  for (let j = 0; j < toolResults.length; j++) {
    try {
      const data = JSON.parse(toolResults[j].content) as Record<string, unknown>
...
function analyzeNarrativeProgress(state: {
  ...
  narrativeStallLevel?: number
  toolCalls?: KpToolCall[]
}): { nextStallLevel: number; shouldForceClue: boolean; shouldForceScene: boolean } {
  const intent = state.playerIntent || 'narrative'
  let stallLevel = state.narrativeStallLevel || 0
  const toolCalls = state.toolCalls || []

  const progressTools = ['grant_clue', 'transition_scene', 'skill_check']
  let hasProgressTool = false
  if (toolCalls && toolCalls.length > 0) {
    for (let i = 0; i < toolCalls.length; i++) {
      const name = toolCalls[i].name || ''
      for (let j = 0; j < progressTools.length; j++) {
  ...
  const shouldForceClue = stallLevel >= 2
  const shouldForceScene = stallLevel >= 4
...
function createPlanNode(agentKind: string) {
...
export function createKPGraph(invokeLLM: InvokeLLM) {
 ...
 */
export async function invokeKPAgent(messages: KpMessage[], invokeLLM: InvokeLLM, storyContext?: Record<string, unknown> | null): Promise<KpGraphResult> {
  ...
  const result = (await graph.invoke(initialState as KpAgentState)) as KpAgentState
  return {
