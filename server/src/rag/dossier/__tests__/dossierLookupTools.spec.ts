/**
 * DossierLookupTools 直测（架构走查候选 3）：档案域查证工具执行器。
 *
 * 四个查证工具（scene_list / scene_dossier / lexical_search / verify_original）的
 * 执行语义此前住在房间域（RoomService 闭包 → 候选 1 收编进 turnKnowledge），但
 * 它们的回包文案与数据源全在档案域——本 spec 直测档案域执行器 runStoryLookup
 * （经与生产 turnKnowledge.buildStoryLookup 活门一致的薄委托形态调用，#74）：
 ...
 * 「回合内要不要提供查证工具」的 workflow 门决策单源在 turnKnowledge，
 * 断言在 turnKnowledge.spec（含 storyId=null 分支，自本票起由彼处覆盖）。
 */
...
import { runStoryLookup, type StoryLookupHandler, type StoryLookupInput } from '../dossierLookupTools.js'
...
/** 执行器直连入口：与生产活门（turnKnowledge.buildStoryLookup 的薄委托分支）同形态——
 *  工厂门已单源归 turnKnowledge（#74 删除档案域静态死工厂），此处只测执行语义。 */
function makeLookup(input: StoryLookupInput): StoryLookupHandler {
...
describe('runStoryLookup — scene_list / scene_dossier（档案查询）', () => {
...
describe('runStoryLookup — lexical_search（词面检索）', () => {
...
describe('runStoryLookup — verify_original（原文查证，活值 getter）', () => {
...
describe('runStoryLookup — 失败降级（回合不因查证中断）', () => {
