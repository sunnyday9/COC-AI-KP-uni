...
# 过滤 + 自定义路径
npm run export:kp-context -- --out runs/demo.jsonl --room room_xxx
...
```jsonc
{
  "meta": {
    ...
    "origin": "room | orphan-wire",    // 在场房间 / 已回收房间的采样（save 源已随 /api/saves* 退役删除，#92）
    "roomId": "…", "userId": 7, "storyId": "…", "storyName": "…",
...
- 金样本 fixture：`test/fixtureDb.ts` 以 e2e demo 剧本《旧图书馆的铜钥匙》为蓝本
  构建确定性 sqlite 库（房间 wire×2 / 无采样回合 / 孤儿采样）；
